...
class CircularDynamicMasking(MaskingStrategy):
    ...
    def build_mask(self, objects_info: np.ndarray, original_image_shape: tuple,
                    target_shape: tuple, original_image: np.ndarray | None = None, annotations=None) -> tuple[np.ndarray, list]:
    ...
    def _adapt_and_filter(self, objects_info, original_shape, target_shape, flux_threshold):
        ...
        for x_coord, y_coord, flux in objects_info:
            if flux <= flux_threshold:
                continue
            adapted.append(((x_coord * target_width) / original_width,
                             (y_coord * target_height) / original_height, flux))
        return adapted
