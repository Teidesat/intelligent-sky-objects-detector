...
#         return False  # Change needed to be aligned with WeightedBCELoss, as it expects Logits. It allows combination with WeightedBCELoss in a multi-task loss scenario.

    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        y_true = y_true.float()
        y_pred = y_pred.float()
        # Change needed to work with logits instead of probabilities, as the model outputs logits:
